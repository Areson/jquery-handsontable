...
WalkontableTableRenderer.prototype.render = function() {
  ...
  var visibleColIndex, totalRows = this.instance.getSetting('totalRows'),
    totalColumns = this.instance.getSetting('totalColumns'),
    columnsToRender = this.wtTable.getRenderedColumnsCount(),
    adjusted = false,
    workspaceWidth, rowsToRender = this.wtTable.getRenderedRowsCount();
...
WalkontableTableRenderer.prototype.markIfOversizedColumnHeader = function(col) {
  var
    level = this.instance.getSetting('columnHeaders').length,
  ...
  while (level) {
    ...
    //currentHeaderHeight = dom.innerHeight(currentHeader) - 1;
