 ...
 *
 * Date: Tue May 05 2015 08:54:24 GMT-0700 (Pacific Daylight Time)
 */
...
//        instance.rootElement.addClass(settings.className);
   ...
   */
  this.version = '0.15.0-beta2'; //inserted by grunt from package.json
};
  ...
  var walkontableConfig = {
    ...
    onCellMouseDown: function (event, coords, TD, wt) {
      ...
      if (!event.isImmediatePropagationStopped()) {
        ...
        // Copy the old cell values as they will get messed with otherwise by "inInSelection"
        var origCoords = {
          row: coords.row,
          col: coords.col
        };

        if (event.button === 2 && instance.selection.inInSelection(origCoords)) { //right mouse button
          //do nothing
        }
...
(function (Handsontable) {
  ...
  function HandsontableAutoColumnSize() {
    ...
    this.determineColumnWidth = function (col, force) {
      ...
      tmp.container.className = instance.rootElement.className + ' htAutoColumnSize' + (force ? ' htOverrideColumnSize' : '');
...
WalkontableHorizontalScrollbarNative.prototype.resetFixedPosition = function() {
  ...
  elem.style.width = dom.outerWidth(this.clone.wtTable.TABLE) + 4 + 'px'; // + 4 + 'px';
};
